...
public class EventNotificationDialog extends JFrame {
  ...
  private void playSoundNotification() {
		...
		if (Configuration.get("NOTIFY_SOUND").equals("DEFAULT"))
			...
			try {
				url =
					new File(Configuration.get("NOTIFY_SOUND").toString()).toURI()
						.toURL(); //Added .toURI to allow conversion to URL
			} catch (Exception ex) {
				url =
					EventNotificationDialog.class.getResource(
						"/ui/beep.wav");
			}
		try { //Per Oracle documentation, the Applet API is deprecated with no replacement
