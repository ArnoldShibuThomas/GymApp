...
public class EventsPanel extends JPanel {
    ...
    /**
     * This is the oveerriden constructor.
     */
    public EventsPanel(DailyItemsPanel parentPanel, JSONTool j) {
        try {
            this.parentPanel = parentPanel;
            jsonTool = j;

            scheduleTool = new AvaliabilityTool(jsonTool.getView(), jsonTool.getUser());

            eventsTable = new EventsTable(scheduleTool);
    ...
    void jbInit() throws Exception {
        ...
        editEventB.setToolTipText(Local.getString("Edit Availability"));
        ...
        ppEditEvent.setText(Local.getString("Edit Availability") + "...");
        ...
        ppRemoveEvent.setText(Local.getString("Remove Availability"));
        ...
        ppNewEvent.setText(Local.getString("New Availability") + "...");
    ...
    void editEventB_actionPerformed(ActionEvent e) {
        EventDialog dlg = new EventDialog(
            App.getFrame(), Local.getString("Edit Availability"), 
            jsonTool,scheduleTool);

        dlg.editDialogSetup(
            (main.java.memoranda.gymAssets.Avaliability) 
            eventsTable.getModel().getValueAt(
                eventsTable.getSelectedRow(),EventsTable.EVENT));
    ...
    void newEventB_actionPerformed(ActionEvent e, String tasktext, Date startDate, Date endDate) {
        ...
        //create the dialog
        EventDialog dlg = new EventDialog(
            App.getFrame(), Local.getString("New Availability"),
            jsonTool,scheduleTool);
    ...
    private void updateEvents(EventDialog dlg, int hh, int mm, String text) {
        ...
        if (dlg.dailyRepeatRB.isSelected()) {
            ...
            if ((sd.getYear() % 4) == 0 && sd.getCalendar()
                .get(Calendar.DAY_OF_YEAR) > 60) {
                period--;
            }
        } else {
            rtype = EventsManager.REPEAT_MONTHLY;
            period = ((Integer) dlg.dayOfMonthSpin
                .getModel().getValue()).intValue();
        }

        EventsManager.createRepeatableEvent(rtype, sd, ed, 
            period, hh, mm, text, dlg.workingDaysOnlyCB.isSelected());
    }

    void removeEventB_actionPerformed(ActionEvent e) {
        String msg;
        Avaliability av = new Avaliability();

        if (eventsTable.getSelectedRows().length > 1) {
            msg = Local.getString("Remove") + " " + eventsTable
            .getSelectedRows().length 
                + " " + Local.getString("Availability") + "\n" 
                + Local.getString("Are you sure?");
        } else {
            av = (Avaliability) eventsTable.getModel().getValueAt(
                eventsTable.getSelectedRow(),
                EventsTable.EVENT);
            msg = Local.getString("Remove Availability") + "\n" 
                + av.getTrainer() + "'s " 
        ...
        int n =
            JOptionPane.showConfirmDialog(
                App.getFrame(),
                msg,
                Local.getString("Remove Availability"),
                JOptionPane.YES_NO_OPTION);
        System.out.println("[Debug]: Button choosen " + n);

        //if the cancel button is selected
        if (n == 1) {
            return;
        } else {
            //if ok button is selected then do this 
            //remove availability from list
            AvaliabilityTool a = eventsTable.getTool();

            a.removeAvaliability(av);
