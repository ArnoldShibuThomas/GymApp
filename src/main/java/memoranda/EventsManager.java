...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	static class Year {
		...
		public int getValue() {
			return Integer.parseInt(yearElement.getAttribute("year").getValue()); //@Todo surround with try catch if necessary
		}
	...
	static class Month {
		...
		public int getValue() {
			return Integer.parseInt(mElement.getAttribute("month").getValue()); //@Todo surround with try catch if necessary
		}
			...
			el.addAttribute(
				new Attribute(
					...
					new CalendarDate(
						d,
						getValue(),
							parseInt(((Element) mElement.getParent())
									.getAttribute("year")
									.getValue()))
						.toString()));
	...
	static class Day {
		...
		public int getValue() {
			return Integer.parseInt(dEl.getAttribute("day").getValue()); //@Todo surround with try catch if necessary
		}
