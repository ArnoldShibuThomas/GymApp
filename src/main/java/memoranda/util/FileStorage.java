...
public class FileStorage implements Storage {
    ...
    private static String fileWeAreIn;
    ...
    public FileStorage() {
        ...
        createNewDirectory();
        System.out.println("[Debug]: Storage for view " + view);
    }

    private void createNewDirectory(){
        //read the user view to a file
        String line = getUserName();
        File f= new File(JN_DOCPATH + "__default/"+line);
        if(!f.exists()){
        boolean temp = f.mkdir();
            if(!temp){
    ...
    public static void saveDocument(Document doc, String filePath) {
        System.out.println("[SaveDocument]: Line 100 saving file to: " + filePath);
        /**
    ...
    public static Document openDocument(String filePath) {
        System.out.println("[SaveDocument]: Line 129 opening file from: " + filePath);
    ...
    public void storeNote(Note note, javax.swing.text.Document doc) {
        ...
        String line = getUserName();
        filename = filename + line + File.separator;
    ...
    public javax.swing.text.Document openNote(Note note) {
        System.out.println("[Test]: Line 209 We are attempting to read note");
        ...
        System.out.println("[Debug]: " + result[0]);
        System.out.println("[Test]: We are reading: " + fileWeAreIn + filename);
        try {
            /*DEBUG*/
   ...
   public String getNotePath(Note note) {
        String filename = JN_DOCPATH + note.getProject().getID() + File.separator + getUserName() + File.separator;
//        CalendarDate d = note.getDate();
        filename += note.getId();//d.getDay() + "-" + d.getMonth() + "-" + d.getYear();
    ...
    public NoteList openNoteList(Project prj) {
        String line = getUserName();
        String fn = JN_DOCPATH + prj.getID() + File.separator + line + File.separator + ".notes";
        if (documentExists(fn)) {
            /*DEBUG*/
            System.out.println(
    ...
    public void storeNoteList(NoteList nl, Project prj) {
        ...
        saveDocument(
            nl.getXMLContent(),
            JN_DOCPATH + prj.getID() + File.separator + getUserName() + File.separator + ".notes");
    }
    /**
    ...
    /***
     * A method for recovering the username of the currently logged-in user
     * for saving and using notes.
     * @return returns String value of the userName we've set to save under
     */
    public String getUserName(){
        String line = "";
        try{
            InputStream in = new FileInputStream("src/main/resources/gymfiles/view.txt");
